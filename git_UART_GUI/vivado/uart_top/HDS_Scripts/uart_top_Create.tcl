...
set xilinxProject "./uart_top.xpr"
set library git_UART_GUI
set unit uart_top
set view struct
set removeIP 0
set removeHDL 0
...
set simulatorType questa
set_property target_simulator [string totitle $simulatorType] [current_project]

set currVer [version -short]
set refVer "2016.1"
set verList [list $refVer $currVer]
set sortedVerList [lsort -increasing -dictionary $verList]
set dotPropertyName "compxlib.compiled_library_dir"
if {$sortedVerList == $verList} {
   set dotPropertyName "compxlib.${simulatorType}_compiled_library_dir"
}

set_property $dotPropertyName "" [current_project]
...
puts "   ./HDS_Scripts/uart_top_fileList.tcl"
set hdlFileList ./HDS_Scripts/uart_top_fileList.tcl
...
lappend constrFiles "$::env(HDS_PROJECT_DIR)/repo1/git_UART_GUI/hds/uart_top/struct.bd.info/Synthesis/Constraints/uart_top.sdc"

foreach constrFile $constrFiles {
   add_files -quiet -fileset [get_property -quiet constrset [current_run]] $constrFile
}
